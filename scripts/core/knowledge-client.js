(function () {
  ...
  function replaceShared(select, items, map, preserve) {
    ...
    const retained = preserve
      ? [...select.querySelectorAll("option")]
          .filter(item => preserve(item.value))
  ...
  async function load() {
    try {
      ...
      const template =
        document.getElementById("edit-template") || document.getElementById("template");
      const level =
        document.getElementById("edit-knowledge-level") ||
      ...
      replaceShared(
        template,
        config.templates,
        item => ({ value: item.code, label: `${item.code} | ${item.name}` }),
        value => !/^[A-Z][A-Z0-9]{0,2}$/.test(value)
      );
      replaceShared(
        level,
        config.levels,
