...
const { saveMediaMetadata } = require("../lib/metadata-repository");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const requestedKind =
      body?.title === PAYMENT_SETTINGS_TITLE ? "photo" : normalizeMediaKind(body?.kind);
    ...
    const recordMetadata =
      body?.paywallEnabled === undefined
        ? {}
        : { paywall: { enabled: Boolean(body.paywallEnabled), price: paywallPrice } };
    const payload = {
      title: body?.title || body?.fileName || "未命名作品",
      description:
        Object.keys(recordMetadata).length > 0
          ? mediaMetadata.append(body?.description || "", recordMetadata)
          : body?.description || "",
      kind: requestedKind,
      url: body.url,
      path: body.path,
    ...
    if (data?.id && Object.keys(recordMetadata).length > 0) {
      await saveMediaMetadata(data.id, recordMetadata);
    }
